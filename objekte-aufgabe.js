...
/**
 * Returns average age of all students in given array
 * @param {[{}]} studentsArray 
 * @returns number
 */
function averageAgeOfStudents(studentsArray){
  let sumAgeofStudents = 0
  for (let student of students){
    if ("age" in student){
      sumAgeofStudents += student.age
    }
  }
  console.log(sumAgeofStudents);
  const averageAge = sumAgeofStudents/students.length
  return averageAge
}
console.log("average age ist: " + averageAgeOfStudents(students));
...
//Code hatte nicht mehr funktioniert, da mit einem key gerechnet werden
// sollte der undefined war. Code wurde umgeschrieben, damit überprüft wird
// ob der key "age" existiert bei dem studenten
